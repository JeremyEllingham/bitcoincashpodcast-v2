...
sidebar_position: 11
title: What is the maximum Bitcoin Cash blocksize?
---

import ReactPlayer from "react-player/youtube";

The current Bitcoin Cash maximum blocksize is 32 MB. Some miners have locally set "soft limits" at lower amounts (typically 8 MB) due to the current infrequency of greater traffic.

In May 2024, the Bitcoin Cash network will upgrade to a new [Adaptive Blocksize Upgrade Limit](https://gitlab.com/0353F40E/ebaa). Starting from a guaranteed minimum of 32 MBs & with no (eventual) upper bound, it will increase or decrease blocksizes according to real network traffic (observed blocksizes). The maximum increase is 2x per year if all blocks are 100% full, but under realistic conditions this is unlikely.

The algorithm intelligently adapts to sudden, large demand spikes by allowing rapid initial increases which it slows if upward momentum is too fast. It also decreases blocksizes if capacity is excessive for long periods of time, to reduce burden on infrastructure providers. Once implemented, the Bitcoin Cash community can spend its energy attacking other problems than debating a perfect size & coordinating an upgrade, hopefully in perpetuity.
...
While having no limit is foolish, the BCH community does recognise that the coordination cost of raising the limit each time is substantial & potential for splitting the community over a divisive issue does exist. To address these concerns, an automatically adapting blocksize will be introduced in May 2024.
