...
- **Requires larger blocks**: For the Lightning Network to function effectively, it requires at least 133 MB blocks. This point was made in [the original Lightning Network white paper](https://lightning.network/lightning-network-paper.pdf). Note that this assumes no usage of the chain for anything other than opening channels (no closing channels, no channel disputes, no regular on-chain payments, no other sidechain solutions) so in reality blocksizes would need to be even larger.
